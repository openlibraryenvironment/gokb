...
class PackageService {
   ...
   */
  def findCorrectPackage (Map<String, Long> retire_packages, String package_name, boolean incremental) {
	...
	if (!pkg) {
	  ...
	  if (!incremental) {
		...
		// Check to see if the retired_packages contains data for this package_name
		if (retire_packages.get(package_name) == null) {
		  
		  // Add the package for retirement here.
		  retire_packages.put(package_name, pkg.id)
		  
		  // Get all IDs attached to the package.
		  Set<Identifier> pkIds = []
		  pkIds.addAll(pkg.ids.findAll { Identifier the_id ->

  ...
  <g:form controller="workflow" action="action" method="post" class='action-form' >
	  <table class="table table-striped table-condensed table-bordered">
	    ...
	    <tbody>
	      <g:each in="${rows}" var="r">
	        <g:set var="r" value="${r}"/>
	        <g:set var="readonly" value="${ r.respondsTo('isSystemComponent') && r.isSystemComponent() }" />
	        <tr class="${++counter==det ? 'success':''}">
	          <!-- Row ${counter} -->
	          <td>
	            <g:if test="${!readonly && r.respondsTo('availableActions')}">
	            ...
	            <g:else>
	              <input type="checkbox" title="${ readonly ? 'Component is read only' : 'No actions available' }" disabled="disabled" readonly="readonly" />
	            </g:else>
	          </td>
	          <g:each in="${qbeConfig.qbeResults}" var="c">
	            <td>

	  	...
	  	<h1 class="page-header">Title Transfer</h1>
			<div id="mainarea" class="panel panel-default">
				...
				<div class="panel-body">
					<h3>${d.activityName} - ${d.status?.value}</h3>
	        ...
	        <p>
	        	Will be transferred from their current publisher to ${newPublisher.name}.
	        	<span style="background-color:#FF4D4D;">Current tipps shown with a red background</span> will be deprecated. <span style="background-color:#11bb11;">New tipps with a green background</span> will be created by this transfer.
	        </p>
				</div>
	        ...
	        <tbody>
	          <g:each in="${tipps}" var="tipp">
	            <tr style="background-color: ${tipp.type=='NEW'?'#4DFF4D':'#FF4D4D'};">
	              <td><g:if test="${tipp.type=='CURRENT'}"><input name="addto-${tipp.id}" type="checkbox" checked="checked"/></g:if></td>
	              <td>${tipp.type}</td>
	              ...
	              <td>${tipp.endIssue}</td>
	              <td><g:if test="${tipp.type=='CURRENT'}"><input name="close-${tipp.id}" type="checkbox" checked="checked"/></g:if></td>
	          ...
	          <p>Use the following form to indicate the package and platform for new TIPPs. Select/De-select TIPPS above to indicate</p>

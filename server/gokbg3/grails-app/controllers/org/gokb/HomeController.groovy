  ...
  def calculate() {
    ...
    widgets.each {widget_name, widget_data ->
      ...
      for ( int i=0; i<12; i++ ) {
        ...
        def qry_pars = [type: component_name, year: calendar.get(Calendar.YEAR), month: calendar.get(Calendar.MONTH)]
        def comp_stats = ComponentStatistic.executeQuery("from ComponentStatistic where componentType = :type and year = :year and month = :month", qry_pars, [readOnly: true])[0]

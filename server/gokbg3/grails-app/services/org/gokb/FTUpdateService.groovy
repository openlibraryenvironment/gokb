      ...
      case org.gokb.cred.TitleInstancePackagePlatform:
        ...
        if (kbc.title?.niceName == 'Journal') {
          ...
          coverage_src.each { tcs ->
            def cst = [:]
            if (tcs.startDate) cst.startDate = dateFormatService.formatTimestamp(tcs.startDate)
            cst.startVolume = tcs.startVolume ?: ""
            cst.startIssue = tcs.startIssue ?: ""
            if (tcs.endDate) cst.endDate = dateFormatService.formatTimestamp(tcs.endDate)
        ...
        if (kbc.title?.niceName == 'Book') {
          ...
          if (kbc.title?.dateFirstInPrint) result.titleDateFirstInPrint = dateFormatService.formatTimestamp(kbc.title.dateFirstInPrint)
          if (kbc.title?.dateFirstOnline) result.titleDateFirstOnline = dateFormatService.formatTimestamp(kbc.title.dateFirstOnline)
        ...
        if (kbc.dateFirstOnline) result.dateFirstOnline = dateFormatService.formatTimestamp(kbc.dateFirstOnline)
        if (kbc.dateFirstInPrint) result.dateFristInPrint = dateFormatService.formatTimestamp(kbc.dateFirstInPrint)
        if (kbc.accessStartDate) result.accessStartDate = dateFormatService.formatTimestamp(kbc.accessStartDate)
        if (kbc.accessEndDate) result.accessEndDate = dateFormatService.formatTimestamp(kbc.accessEndDate)
        if (kbc.lastChangedExternal) result.lastChangedExternal = dateFormatService.formatTimestamp(kbc.lastChangedExternal)

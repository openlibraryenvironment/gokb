...
class GroupController {
  ...
  def index() {
    ...
    if ( params.id ) {
      ...
      RefdataValue closedStat = RefdataCategory.lookup('ReviewRequest.Status', 'Closed')
      RefdataValue delStat = RefdataCategory.lookup('ReviewRequest.Status', 'Deleted')
      RefdataValue inactiveStat = RefdataCategory.lookup('AllocatedReviewGroup.Status', 'Inactive')
      ...
      String cg_review_tasks_hql = ''' from ReviewRequest as rr where (
                                        (
                                          rr.allocatedTo in (
                                            select u from CuratoryGroup as cg
                                            join cg.users as u
                                            where cg = :group
                                          )
                                          or rr.componentToReview.id in (:cgcomponents)
                                        ) or exists (
                                          select arc from AllocatedReviewGroup as arc
                                          where arc.review = rr
                                          and arc.group = :group
                                          and arc.status != :inactive)
                                        )

...
class OaiController {
  ...
  def identify(options) {
    ...
    def first_timestamp = KBComponent.executeQuery("select ${options.oaiConfig.lastModified} from ${options.className} as o ORDER BY ${options.oaiConfig.lastModified} ASC".toString(), [], [max:1, readOnly:true])[0]
    def last_timestamp = KBComponent.executeQuery("select ${options.oaiConfig.lastModified} from ${options.className} as o where ${options.oaiConfig.lastModified} IS NOT NULL ORDER BY ${options.oaiConfig.lastModified} DESC".toString(), [], [max:1, readOnly:true])[0]

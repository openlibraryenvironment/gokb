...
class RefdataValue {
  ...
  static def refdataFind(params) {
    ...
    def query = "from RefdataValue as rv where rv.useInstead is null and lower(rv.value) like :qry"
    def query_params = [qry: "%${params.q.toLowerCase()}%"]

    if ( ( params.filter1 != null ) && ( params.filter1.length() > 0 ) ) {
      query += " and rv.owner.desc = :owner order by rv.sortKey, rv.description"
      query_params.put('owner', params.filter1)
    }

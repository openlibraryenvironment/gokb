  ...
  def ensureStats(int months = 12, int offset = 0, boolean force_update = false) {
    ...
    components.each { c ->
      ...
      for ( int i; i <= to_substract; i++ ) {
        ...
        def existing_stats = ComponentStatistic.executeQuery("from ComponentStatistic where componentType = :ct and month = :mo and year = :yr", [ct: c, mo: period_month, yr: period_year], [readOnly: true])

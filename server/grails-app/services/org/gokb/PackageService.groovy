...
'''
  */
      ...
      if (item) {
        try {
          ...
          if (force || (Duration.between(item.lastUpdated.toInstant(), Instant.now()).getSeconds() > 30 && (!currentCacheFile || item.lastUpdated > currentCacheDate))) {
            ...
            def pkg_ids = Identifier.executeQuery("select i.namespace.value, i.namespace.name, i.value, i.namespace.family from Identifier as i, Combo as c where c.fromComponent = :fc and c.type = :ct and c.toComponent = i and c.status = :cs", [fc: item, ct: refdata_ids, cs: status_active], [readOnly: true])
  ...
  private def getTitleIds(Long title_id) {
    ...
    def result = Identifier.executeQuery("select i.namespace.value, i.value, i.namespace.family, i.namespace.name from Identifier as i, Combo as c where c.fromComponent.id = :ti and c.type = :ctype and c.toComponent = i and c.status = :cs", [ti: title_id, ctype: refdata_ids, cs: status_active], [readOnly: true])
    result
  }
  ...
  private def getTippIds(Long tipp_id) {
    def refdata_ids = RefdataCategory.lookupOrCreate('Combo.Type', 'KBComponent.Ids');
    def status_active = RefdataCategory.lookupOrCreate(Combo.RD_STATUS, Combo.STATUS_ACTIVE)
    def result = Identifier.executeQuery("select i.namespace.value, i.value, i.namespace.family, i.namespace.name from Identifier as i, Combo as c where c.fromComponent.id = :tipp and c.type = :ctype and c.toComponent = i and c.status = :cs", [ti: tipp_id, ctype: refdata_ids, cs: status_active], [readOnly: true])
    result
  }
